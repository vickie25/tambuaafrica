  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Fallback to empty array - Supabase may be unavailable
...
export const useUnreadInquiriesCount = () => {
  const queryClient = useQueryClient();
  const newInquiryStatuses = ["unread", "pending", "synced", "sync_failed"];
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        return { revenue: 0, inquiriesCount: 0 };
      }
    },
    staleTime: 1000 * 60 * 5,
    retry: 1,
